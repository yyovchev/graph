...
class graph : public vector<node<T>>
{
    ...
    public:
        bool add_node(T node_name);
        bool add_edge(T node_name_1, T node_name_2, int len = 1);
        void add_len(T node_1, T node_2, int len);
        int get_index(T name);
        T get_name(int index);
        string print_v();
        string print_g();
        string print_l();
        vector<T> DFS();
        vector<T> DFS(T strtIndex);
        vector<T> BFS();
        int Dijkstras_algo(T start_node, T end_node);

    private:
        void dfs_indx(int index, vector<T> &list_of_visited, bool *visited);
...
vector<T> graph<T>::BFS(){
    ...
    while (!que.empty()){
        ...
        for_each(this->begin(), this->end(), [&que, this, &visited, value](node<T> arg){
            if (arg.index == value){
                for_each(arg.neighbours.begin(), arg.neighbours.end(), [&que, this, &visited](int arg){
            //  for_each(arg.neighbours.begin(), arg.neighbours.end(), [&visited, this, &list_of_visited](int arg){
